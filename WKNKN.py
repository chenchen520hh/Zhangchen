...
if __name__ == "__main__":
    DTI=np.loadtxt('whole_data/mat_drug_protein.txt')
    ...
    for i in range(DTI.shape[0]):
        for j in range(DTI.shape[1]):
            if DTI[i][j]==1:
                num1+=1
    print(num1)
    frequent_no_zero=num1/(DTI.shape[0]*DTI.shape[1])  #原始数据集非零元素比例 0.0018
    ...
    for i in range(predict_Y.shape[0]):
        for j in range(predict_Y.shape[1]):
            if predict_Y[i][j]!=0:
                num_float+=1
    print(num_float)
    frequent_no_zero=num_float/(predict_Y.shape[0]*predict_Y.shape[1]) #补全后非零元素比例0.0493
