...
if __name__=="__main__":
    # 药物相似性矩阵融合
    drug_drug_interaction=np.loadtxt('whole_data/mat_drug_drug.txt')#药物-药物相互作用矩阵[708,708]
    drug_disease_association=np.loadtxt('whole_data/other_information_to_be_used/mat_drug_disease.txt')#药物疾病[708,5603]
    drug_sideeffect_association=np.loadtxt('whole_data/other_information_to_be_used/mat_drug_se.txt')#药物副作用 [708,4192]
    drug_drug_chemistry_similarity=np.loadtxt('whole_data/Similarity_Matrix_Drugs.txt') #药物化学结构[708,708]
    # 前三个矩阵需要求jaccard相似度
    ...
    # 靶点相似性矩阵融合
    target_disease_association=np.loadtxt('whole_data/other_information_to_be_used/mat_protein_disease.txt')
    target_target_interaction=np.loadtxt('whole_data/mat_protein_protein.txt')
    target_target_sequence_similarity=np.loadtxt('whole_data/Similarity_Matrix_Proteins.txt')
    # 前两个矩阵需要求jaccard相似度
