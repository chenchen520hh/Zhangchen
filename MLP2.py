...
parser.add_argument('-f', '--fold', type=int, default=2, help='Number of folds')
...
# 读取数据
index_1 = np.loadtxt('divide_test/index_1.txt')#10折交叉验证中1的编号
index_0 = np.loadtxt('divide_test/index_0.txt')#10折交叉验证中0的编号
index = np.hstack((index_1, index_0)) #合并正负样本的编号
#加载特征向量和标签
drug_feature = np.loadtxt('CL-result-of-VGAE/embedding'+str(fold)+'.txt')[0:n, :]#前n行(0到n-1行)是学到的药物的特征向量
target_feature = np.loadtxt('CL-result-of-VGAE/embedding'+str(fold)+'.txt')[n:, :]#后面是靶标的特征向量
label=np.loadtxt('whole_data/mat_drug_protein.txt') #标签矩阵
...
# 训练模型
model.train()
for epoch in range(1500):  # 迭代次数可以调整
...
print('Recall:', recall_score(test_output, y_pred_label))
print('F1 Score:', f1_score(test_output, y_pred_label))
print('MCC:', matthews_corrcoef(test_output, y_pred_label))

np.savetxt('CL-result-of-VGAE/VGAE' + str(fold) + '.txt', y_pred)
np.savetxt('CL-result-of-VGAE/VGAE_test_index' + str(fold) + '.txt', test_index)
#ROC 和 PR 曲线数据
# auc_score = roc_auc_score(test_output, y_pred)
# aupr_score = average_precision_score(test_output, y_pred)
# fpr, tpr, _ = roc_curve(test_output, y_pred)
# precision, recall, _ = precision_recall_curve(test_output, y_pred)
#
# # 导出数据到Excel文件
# # 保存结果到Excel文件
# output_file = f'results_fold_ptGCN_{fold}.xlsx'
# with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
#     # 保存AUC和AUPR
#     auc_aupr_df = pd.DataFrame({'AUC': [auc_score], 'AUPR': [aupr_score]})
#     auc_aupr_df.to_excel(writer, sheet_name='AUC and AUPR', index=False)
#
#     # 保存ROC曲线数据
...
#     pr_df.to_excel(writer, sheet_name='PR Curve', index=False)
#
# print('Results exported to results_fold_ptGCN_'+str(fold)+'.xlsx')
